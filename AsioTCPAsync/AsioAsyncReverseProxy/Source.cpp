...
public:
	...
	void Connect(std::string szHost, int nPort, boost::shared_ptr<XBOOSTAsyncProxySocket>& pXPeerSocket)
	{
		std::cout << "Client: " << shared_from_this().use_count() << std::endl;
		m_pSocket->async_connect(*GetEndpoint(szHost, nPort),
...
{
public:
	XBOOSTAsyncProxyServer(int nProxyServerPort, std::string szServerHost, int nServerPort)
		: m_pIOContext(new boost::asio::io_context()),
		m_pWork(new boost::asio::io_context::work(*m_pIOContext)),
		m_pThreadGroup(new boost::thread_group())
	{
		std::cout << "XBOOSTAsyncProxyServer" << std::endl;
		m_pEndpoint1 = GetEndpoint(nProxyServerPort);
		m_pAcceptor = boost::make_shared<boost::asio::ip::tcp::acceptor>(*m_pIOContext, *m_pEndpoint1);
		//m_pAcceptor(new boost::asio::ip::tcp::acceptor(*m_pIOContext))
	...
	void Start()
	{
		...
		//const char* szNoThreads = getenv_s("HCS_BOOST_NO_THREAD", );
...
void _tmain_removethis()
{
	//XBOOSTAsyncProxyServer xBOOSTAsyncServer(5800, "127.0.0.1", 2000);
	//xBOOSTAsyncServer.Start();
